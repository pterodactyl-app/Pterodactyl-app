...
class LoginRoyaleHostingPage extends StatefulWidget {
  @override
  _LoginRoyaleHostingPageState createState() => new _LoginRoyaleHostingPageState();
}

bool checkValue = false;

class _LoginRoyaleHostingPageState extends State<LoginRoyaleHostingPage> {
  ...
  Future<String> _getApiToken(String username, String password) async {
    if(username.isNotEmpty && password.isNotEmpty) {
      await SharedPreferencesHelper.setString('apiUser_royalehosting', username);
      http.Response response = await http.post(
        "https://pterodactyl.rhmc.nl/api/app/user/auth/token",
        headers: {
  ...
  _onChanged(bool value) async {
    ...
    setState(() {
      ...
      sharedPreferences.setString("apiPass_royalehosting", _passController.text);
      getCredential();
    });
  }
  ...
  getCredential() async {
    ...
    setState(() {
      ...
      if (checkValue != null) {
        if (checkValue) {
          _userController.text = sharedPreferences.getString("apiUser_royalehosting");
          _passController.text = sharedPreferences.getString("apiPass_royalehosting");
        } else {
          _userController.clear();
          _passController.clear();
    ...
    if (_userController.text.length != 0 || _passController.text.length != 0) {
      if (_passController.text.isEmpty) {
        Navigator.of(context)
            .pushNamedAndRemoveUntil('royalehosting/login', (Route<dynamic> route) => false);
        return false;
      }

      String _token = await SharedPreferencesHelper.getString('api_royalehosting_Key');
      ...
      if (response.statusCode == 200) {
        Navigator.of(context)
            .pushNamedAndRemoveUntil('royalehosting/home', (Route<dynamic> route) => false);
      } else {
        showDialog(
          context: context,
          barrierDismissible: false,
